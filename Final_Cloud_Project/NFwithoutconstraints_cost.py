...
while i<=23:
  ...
  for j in range(100):
    ...
    flag = 0;
    for x in range(nextfittemp,len(list_of_cpus)):
     if temp_mem < list_of_cpus[x] and temp_cpu < list_of_memories[x] and temp_net < list_of_networks[x] and list_of_cpus[x]-temp_cpu>0 and list_of_memories[x]-temp_mem>0 and list_of_networks[x]-temp_net>0:
       flag=1;
       usage_list[x] = 1;
  ...
  for x in usage_list:
   sum = sum + x;
  print "Next Fit on the System for", i , " th Hour", sum*cost , "\n";
  k=0;
  final = 0;
  finalone =0;
  finaltwo = 0;
  ...
  print "Power utilization for", i ,"th Hour",finalthree*power,"Hertz \n";  
  list_of_cpus=[];
  list_of_memories=[];
  list_of_networks=[];
  usage_list=[];
