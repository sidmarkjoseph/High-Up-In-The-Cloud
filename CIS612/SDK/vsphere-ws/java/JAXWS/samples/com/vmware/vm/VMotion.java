			...
			for (String prop : hostprops.keySet()) {
				if (prop.equalsIgnoreCase("name")
						&& hostprops.get(prop).toString()
								.equalsIgnoreCase(sourceHost)) {
					//System.out.println("Hostname:" + hostprops.get("name"));
					//writer.append(HOST_NAME + " : " + hostprops.get(HOST_NAME)+"\n");
                                     try{
                                     while(true)
                                      {
					if((Integer)(hostprops.get("summary.quickStats.overallMemoryUsage"))>100)
                                              {       if (customValidation()) {
            propCollectorRef = serviceContent.getPropertyCollector();
            //perfManager = serviceContent.getPerfManager();
            migrateOrRelocateVM();
             break;
        }}System.out.println("Sleeping 120 seconds...");
            Thread.sleep(120 * 1000);}}catch(Exception e){}
             
					//writer.append(HOST_CPU + " : " + hostprops.get(HOST_CPU)+"\n");
					//System.out.println(hostprops.get("summary.quickStats.overallMemoryUsage")+"MB");
        }}}
