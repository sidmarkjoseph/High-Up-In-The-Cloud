...
public class Requirement2 extends ConnectedVimServiceBase {
    ...
    void displayValues(List<PerfEntityMetricBase> values,
       ...
       try{
         //File file = new File ("smjoseph384284085.txt");  
         //file.createNewFile();   
        ...
        for (int i = 0; i < values.size(); ++i) {
            ...
            for (int vi = 0; vi < listpems.size(); ++vi) {
                /*PerfCounterInfo pci = counters.get(new Integer(listpems.get(vi).getId()
                                .getCounterId()));
    ...
    void doRealTime() throws IOException, RuntimeFaultFaultMsg, InterruptedException {
      ...
      try{
       while(true)
       { 
          File file = new File("Requirement2.txt");
        file.createNewFile();
         FileWriter writer = new FileWriter(file,true);
        List<String> hostSystemAttributesArr = new ArrayList<String>();
		...
		for (ManagedObjectReference host : hosts.keySet()) {
			...
			for (String prop : hostprops.keySet()) {
				if (prop.equalsIgnoreCase("name")
					...
					writer.write("Hostname:" + hostprops.get("name"));
                                     writer.write("\n");
					...
					writer.write(hostprops.get("summary.quickStats.overallCpuUsage")+"Mhz");
                                     writer.write("\n");
					//writer.append(HOST_CPU + " : " + hostprops.get(HOST_CPU)+"\n");
					writer.write(hostprops.get("summary.quickStats.overallMemoryUsage")+"MB");
                                     writer.write("\n");
        }}}System.out.println("Sleeping 10 seconds...");writer.flush();writer.close();
            Thread.sleep(10 * 1000);           }}
      catch(Exception e)
      {}
        /*if (vmmor != null) {
            List<PerfCounterInfo> cInfo = getPerfCounters();
            List<PerfCounterInfo> vmCounters = new ArrayList<PerfCounterInfo>();
            for (int i = 0; i < cInfo.size(); ++i) {
                if ((cInfo.get(i).getGroupInfo().getKey())!=null) {
                    vmCounters.add(cInfo.get(i));
                }
              }    
            ...
            Map<Integer, PerfCounterInfo> counters =
             ...
             PerfCounterInfo pcInfo = vmCounters.get(110);
             counters.put(new Integer(pcInfo.getKey()), pcInfo);
             System.out.println(pcInfo.getNameInfo().getSummary());
